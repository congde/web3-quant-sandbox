...
export default function BacktestLearningPage() {
  ...
  return <TradingPageShell eyebrow="BACKTEST SYSTEM · 8 类公式 → 方法 → 验证" title="回测学堂" description="系统学习 28 个回测公式和标准方法：数据样本、仓位成交、净值成本、基准指标、交易质量、稳健性与过拟合修正。" actions={<><Button icon={<BookOutlined />} onClick={() => navigate("/academy")}>返回学堂</Button><Button icon={<ExperimentOutlined />} onClick={() => navigate("/kline-learning")}>复习 K 线课程</Button></>} aside={<QuantGlowCard className="backtest-progress-card"><span>互动实验进度</span><strong>{lesson + 1} / {LESSONS.length}</strong><div><i style={{ width: `${(lesson + 1) / LESSONS.length * 100}%` }} /></div><small>公式手册：8 类 / 28 个公式</small><small>当前实验：{LESSONS[lesson].title}</small></QuantGlowCard>}>
    <LearningCourseNav />
    <section className="learning-full-width">
      <main className="backtest-learning-main"><FormulaHandbook domain="backtest" /><QuantGlowCard title={<SectionHeader title={LESSONS[lesson].title} description="概念、方法、交互示例和适用边界" />} badge={<StatusPill tone="profit">基础课程</StatusPill>}>{LABS[lesson]}</QuantGlowCard><QuantGlowCard title={<SectionHeader title="理解检查" description="判断你是否掌握本课的核心边界" />} badge={<StatusPill tone="ai">1 题</StatusPill>}><strong className="backtest-quiz-question">{quiz.question}</strong><div className="backtest-quiz-options">{quiz.options.map((option, index) => <button type="button" key={option} className={answer === index ? (index === quiz.answer ? "correct" : "wrong") : ""} onClick={() => setAnswer(index)}><i>{String.fromCharCode(65 + index)}</i><span>{option}</span>{answer === index ? (index === quiz.answer ? <CheckCircleFilled /> : <CloseCircleFilled />) : null}</button>)}</div>{answer !== null ? <div className={`backtest-quiz-feedback ${answer === quiz.answer ? "correct" : "wrong"}`}><strong>{answer === quiz.answer ? "回答正确" : "这个结论越过了证据边界"}</strong><span>{quiz.reason}</span></div> : null}</QuantGlowCard><div className="backtest-lesson-actions"><Button disabled={lesson === 0} onClick={() => move(lesson - 1)}>上一课</Button>{lesson < LESSONS.length - 1 ? <Button type="primary" onClick={() => move(lesson + 1)}>下一课 <ArrowRightOutlined /></Button> : <Button type="primary" onClick={() => navigate("/risk-learning")}>继续风控课程 <ArrowRightOutlined /></Button>}</div></main>
    </section>
  </TradingPageShell>;
}
